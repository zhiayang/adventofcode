...
int main()
{
	...
	for(auto [ prog, progs ] : input)
	{
		auto node = nodes[prog];
		for(auto l : progs)
			node->links.push_back(nodes[l]);
	}

	std::function<void (Node*, std::set<int>&)> getsz = [&getsz](Node* n, std::set<int>& seen) {
		if(seen.find(n->id) != seen.end())
			return;
	...
	// make a list of ids that did not appear in 'seen'.
	// goto???
	int count = 1;	// 1 including group 0
	...
	while(true)
	{
		...
		if(notseen.size() > 0)
			getsz(nodes[*notseen.begin()], seen), count++;
